...
function Mc(jid, password, host, participants) {
    ...
    self.on('message', function(message){
        ...
        const matchResult = message.getChild('body').getText().match(new RegExp(daytimeRequest + '(\\d+)?'));
        // if it is a daytime duration request
        if (matchResult){
            if (matchResult[1]){
              daytime = matchResult[1] * 1000;
            };
            // send back the duration of the daytime
