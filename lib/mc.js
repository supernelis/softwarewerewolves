...
function Mc(jid, password, host, participants) {
    ...
    this.__defineGetter__('players', function(){
        return players.map(function(p){return p.nickname});
    });
    ...
    function receive_participant(participant) {
    ...
    self.on('message', function(message){
        ...
        if (body){
            ...
            } else if (werewolfMatchResult && message.type == 'chat'){
                const nickname = message.from.match(new RegExp('^' + room_jid + '\/(.+)$'))[1];
                players.forEach(function(p){
                    if (p.nickname == nickname){
                        p.role = WEREWOLF;
                    }});
                msg.c('body').t(magicStrings.getMagicString('DESIGNATED_AS_WEREWOLF'));
                self.client.send(msg);
            }
        }
    });
    ...
    self.on(PRESENCE_MSG_FROM_PARTICIPANT, function(participant, type){
        if (!type){
            ...
            receive_participant(participant, participant + 'has abandoned the game');
        }
    });
    ...
    var arrival = function(participant){
        ...
        receive_participant(participant);
    };
            ...
            if (status){
                ...
                if (return_code == 201 || return_code == 110){
                    ...
                    setTimeout(function(){
                        ...
                        msg.c('body').t('Please vote who should be hanged: ' + players.map(function(p){return p.nickname}));
                        self.client.send(msg);
                    }, 150000);
                } else {
