...
function Mc(jid, password, host, participants) {
    ...
    self.on('message', function(message){
        ...
        const daytimeRequest = magicStrings.getMagicString('DAYTIME');
        const nighttimeResponseParts = magicStrings.getMagicString('NIGHTTIME_RESPONSE');
        const nighttimeRequest = magicStrings.getMagicString('NIGHTTIME');
        const daytimeRequestMatchResult = message.getChild('body').getText().match(new RegExp(daytimeRequest + '(\\d+)?'));
        const nighttimeRequestMatchResult = message.getChild('body').getText().match(new RegExp(nighttimeRequest + '(\\d+)?'));
        var msg;
        if (daytimeRequestMatchResult || nighttimeRequestMatchResult){
            ...
            if (daytimeRequestMatchResult){
                ...
                msg
                    .c('body')
                    .t(daytimeResponseParts[0] + daytime/1000 + daytimeResponseParts[1])
                ...
                msg
                    .c('body')
                    .t(nighttimeResponseParts[0] + nighttime/1000 + nighttimeResponseParts[1])
    ...
    self.client.on('online', function () {
        ...
        // start the night
        // todo - move to an interval function that toggles between night and day

        util.log('starting the night');
            ...
            if (status){
                ...
                if (return_code == 201 || return_code == 110){
                    ...
                    setTimeout(function(){
                        ...
                        msg.c('body').t('Please vote who should be hanged: ' + participants);
                        self.client.send(msg);
                    }, 150000);
                } else {
