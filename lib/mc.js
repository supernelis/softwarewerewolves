...
function Mc(jid, password, host, participants) {
    ...
    this.__defineGetter__('livePlayers', function(){
        return players.filter(function(p){return p.lives}).map(function(p){return p.nickname});
    });

    this.__defineGetter__('liveVillagers', function(){
       return players.filter(function(p){return p.lives && p.role == VILLAGER}).map(function(p){return p.nickname});
    });
