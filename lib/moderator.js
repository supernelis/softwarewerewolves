...
function Moderator(jid, password, host, toInvite, roomJID) {
    ...
    function startTheDay(victim) {
        ...
        if (self.liveVillagers.length <= self.liveWerewolves.length) {
            ...
            msg = new xmpp.Message({to:roomJID, type:'groupchat', id:'call_for_revenge'});
    ...
    function doAndAcknowledgeTimerCommand(duration, timerId, command, replyPrefix, fn) {
    ...
    function listenForWerewolfElectionDurationCommands(message) {
        ...
        if (requestedDuration) {
            ...
            werewolfElectionId = doAndAcknowledgeTimerCommand(electionDuration, werewolfElectionId, message, WEREWOLF_ELECTION_TIME, startgame)
        }
    }
    ...
    function listenForGameDurationCommands(message) {
        ...
        if (requestedDuration) {
            ...
            gameDurationTimerId = doAndAcknowledgeTimerCommand(gameDuration, gameDurationTimerId, message, GAMETIME, timeIsUp)
        }
    }
    ...
    function listenForDayDurationCommands(message) {
        ...
        if (requestedDuration) {
            ...
            daytime = doAndAcknowledgeTimerCommand(daytime, dayId, message, DAYTIME_REQUEST, function () {
                if (phase == DAY) {
                    self.emit(NIGHTFALL);
                }
            });
