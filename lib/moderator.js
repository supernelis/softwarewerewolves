...
function Moderator(jid, password, host, toInvite) {
    ...
    function startTheDay(victim) {
        ...
        if(self.liveVillagers <= self.liveWolves){
            endGame();
        }
    ...
    function listenForVotes(message) {
        ...
        const body = message.getChild('body');
        if (body && self.livePlayers.indexOf(from) >=0) {
            const text = body.getText();
            const voteMatchResult = text.match(VOTE_REGEXP);
            if (voteMatchResult) {
                const victim = voteMatchResult[1];
                if(victim != from && self.livePlayers.indexOf(victim) >=0) {
                    util.log("from"+ from + " victim "+victim);
                    votes.push(victim);
