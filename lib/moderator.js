...
function Moderator(jid, password, host, toInvite, roomJID) {
    ...
    var electionDurationStillToRun = 30000;
    var werewolfElectionId;

    function listenForWerewolfElectionDurationCommands(message) {
        const body = message.getChild('body');
        if (body) {
            const text = body.getText();
            const werewolfElectionTimeMatchResult = text.match(WEREWOLF_ELECTION_TIME + '(\\d)+$');
            if (werewolfElectionTimeMatchResult) {
                if (werewolfElectionTimeMatchResult[1]) {
                    electionDurationStillToRun = werewolfElectionTimeMatchResult[1] * 1000;
                    const tmp = setTimeout(startgame, electionDurationStillToRun)
                    clearTimeout(werewolfElectionId);
                    werewolfElectionId = tmp;
                }
            }
        }
    }
    ...
    self.client.once('online', function () {
        ...
        werewolfElectionId = setTimeout(startgame, electionDurationStillToRun);
    });
}
