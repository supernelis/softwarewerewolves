...
function Moderator(jid, password, host, toInvite, roomJID) {
    ...
    var werewolfElectionId = setTimeout(startgame, electionDuration);
    ...
    function listenForWerewolfElectionDurationCommands(message) {
        ...
        if (requestedDuration) {
            ...
            werewolfElectionId = executeTimerResetCommandAndAck(electionDuration, werewolfElectionId, message, WEREWOLF_ELECTION_TIME, startgame)
        }
    }
    ...
    function timeIsUp() {
        ...
        msg.c('body').t('this game has been going for long enough. Time is up. Thanks for playing, everyone. See you later');
        self.client.send(msg);
        clearTimeout(dayId);
        clearTimeout(nightId);
    ...
    var gameDurationTimerId = setTimeout(timeIsUp, gameDuration);
    ...
    function listenForGameDurationCommands(message) {
        ...
        if (requestedDuration) {
            ...
            gameDurationTimerId = executeTimerResetCommandAndAck(gameDuration, gameDurationTimerId, message, GAMETIME, timeIsUp)
        }
    }
    ...
    function listenForDayDurationCommands(message) {
        ...
        if (requestedDuration) {
            ...
            dayId = executeTimerResetCommandAndAck(daytime, dayId, message, DAYTIME_REQUEST, function () {
                if (phase == DAY) {
                    self.emit(NIGHTFALL);
                }
            });
    ...
    function listenForNightDurationCommands(message) {
        ...
        if (requestedDuration) {
            ...
            nightId = executeTimerResetCommandAndAck(nighttime, nightId, message, NIGHTTIME_REQUEST, function () {
                if (phase == NIGHT) {
                    self.emit(DAWN);
                }
            });
    ...
    function listenForWerewolves(message) {
        util.log('listening for werewolves');
        const body = message.getChild('body');
        if (body) {
            const text = body.getText();
