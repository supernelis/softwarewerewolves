...
function Moderator(jid, password, host, toInvite) {
    ...
    function listenForPlayerArrival(presence) {
        ...
        if (fromMatchResult) {
            ...
            if (players.length == toInvite.length + 6) {
    ...
    function startgame() {
        ...
        util.log(self.werewolves + ' is the werewolf');
        self.emit(NIGHTFALL);
    }
    ...
    function startBot(nickname, password, player) {
        const jid = nickname + '@' + self.srv;
        return new Bot(new BotXmppHelper(jid, password, self.srv, 'sww@jabber.org', nickname));
    }
    ...
    self.client.once('online', function () {
        ...
        // supplement the players to invite with in-process bots
        startBot('mo_werewolf', 'mo_werewolf', new Player('mo_werewolf')).xmppHelper.on('online', function(){
            invite('mo_werewolf' + '@' + self.srv)});
        startBot('fred_villager', 'fred_villager', new Player('fred_villager')).xmppHelper.on('online', function(){
            invite('fred_villager' + '@' + self.srv)});
        startBot('joligeheidi', 'asjemenou', new Player('joligeheidi')).xmppHelper.on('online', function(){
            invite('joligeheidi' + '@' + self.srv)});
        startBot('bimboooo', 'bimboooo', new Player('bimboooo')).xmppHelper.on('online', function(){
            invite('bimboooo' + '@' + self.srv)});
        startBot('den_john', 'den_john', new Player('den_john')).xmppHelper.on('online', function(){
            invite('den_john' + '@' + self.srv)});
        startBot('pelvis', 'elvispelvis', new Player('pelvis')).xmppHelper.on('online', function(){
            invite('pelvis' + '@' + self.srv)});
