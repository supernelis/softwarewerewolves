...
function GameCoordinator(jid, password, host){
    ...
    self.getCandidateWerewolves = function(){
        return participants.filter(function(player){
            return player.capabilities.indexOf(magicStrings.getMagicString('WEREWOLF')) >= 0;
        }).map(function(p){return p.user});
    };

    self.getCandidateVillagers = function(){
        return participants.filter(function(player){
            return player.capabilities.indexOf(magicStrings.getMagicString('VILLAGER')) >= 0;
        }).map(function(p){return p.user});
    };
            ...
            if (text.match(new RegExp(waittimeRequest))){
                ...
                if (matchResult){
                    ...
                    if (!self.receivedPlayRequestFrom(from)){
                        participants.push(new Player(from, matchResult[1]));
                    }
                } else {
