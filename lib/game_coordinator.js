    ...
    self.client.once('online', function() {
        ...
        self.on('message', function(stanza){
            ...
            if (text.slice(0,8) == 'WAITTIME'){
                util.log('WAITTIME requested')
                var millis = text.slice(8).trim() * 1000;
                if (millis > 0){
                    util.log('setting wait time to ' + millis);
                    wait_time = millis;
                } else {
                    const msg = new xmpp.Message({to: stanza.from});
                    msg.c('body')
                        .t('currently the game co-ordinator is waiting '
                        + (wait_time/1000)
                        + 's before opening the chat room');
