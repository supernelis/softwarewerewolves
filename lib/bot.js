...
function Bot(xmppHelper){
    ...
    xmppHelper.on('wispering',function(from, message){
        ...
        if(message.startsWith(WHO_DO_YOU_WANT_TO_EAT) ){
            ...
            xmppHelper.privatallySpeakInVillage(from,I_EAT + names[randomBelow(names.length)].trim());
    ...
    xmppHelper.on('villagechatter',function(from, message){
        ...
        if(from == self.godname && message.startsWith(REQUEST_VOTE) ){
            var names = getNamesFromMessage(message).remove(xmppHelper.nickname);
            xmppHelper.publiclySpeakInVillage(VOTE + names[randomBelow(names.length)].trim());
        }
    });
